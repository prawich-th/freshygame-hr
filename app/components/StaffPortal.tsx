...
function ParticipantDashboard({ stats, selectedId, setSelectedId, canEdit }: { stats: { total:number;complete:number;pending:number;sports:number } | undefined; selectedId: Id<"participants"> | null; setSelectedId:(id:Id<"participants">|null)=>void; canEdit:boolean }) {
  ...
  return <>
    ...
    {selectedId && <ParticipantDrawer id={selectedId} onClose={() => setSelectedId(null)} canEdit={canEdit}/>} 
  </>;
}
...
function ParticipantDrawer({ id, onClose, canEdit }: { id:Id<"participants">; onClose:()=>void; canEdit:boolean }) {
  ...
  const [files,setFiles] = useState<{profile?:File;nationalId?:File;studentId?:File}>({}); const [busy,setBusy]=useState(false); const [message,setMessage]=useState(""); const [editing,setEditing]=useState(false); const [editKind,setEditKind]=useState<"athlete"|"performer">("athlete");
  ...
  return <><div className="drawer-backdrop" onClick={onClose}/><aside className="drawer"><div className="drawer__head"><h2>Participant record</h2><button onClick={onClose}><X size={17}/></button></div>
    ...
    {editing?<form className="participant-edit" onSubmit={saveParticipant}><div className="form-grid"><EditField name="studentId" label="Student ID" defaultValue={p.studentId} required inputMode="numeric"/><div className="field"><label>Role <span>*</span></label><select className="select" value={editKind} onChange={e=>setEditKind(e.target.value as "athlete"|"performer")}><option value="athlete">Athlete / participant</option><option value="performer">Performer</option></select></div>{editKind==="performer"?<div className="field field--wide"><label>Performer team <span>*</span></label><select className="select" name="performerType" required defaultValue={p.performerType??""}><option value="" disabled>Select team</option><option value="Katakorn">Katakorn</option><option value="Cheerleader">Cheerleader</option><option value="Parade">Parade</option></select></div>:<EditField name="sport" label="Sport / activity" defaultValue={p.sport} required/>}<EditField name="fullNameThai" label="Thai full name" defaultValue={p.fullNameThai} required/><EditField name="nicknameThai" label="Thai nickname" defaultValue={p.nicknameThai}/><EditField name="fullNameEnglish" label="English full name" defaultValue={p.fullNameEnglish} required/><EditField name="nicknameEnglish" label="English nickname" defaultValue={p.nicknameEnglish}/><EditField name="faculty" label="Faculty" defaultValue={p.faculty} required/><EditField name="sex" label="Sex" defaultValue={p.sex}/><EditField name="category" label="Category / type" defaultValue={p.category}/><EditField name="phone" label="Phone" defaultValue={p.phone} type="tel"/><EditField name="email" label="Email" defaultValue={p.email} type="email"/><EditField name="preferredContact" label="Preferred contact" defaultValue={p.preferredContact}/><EditField name="lineId" label="LINE ID" defaultValue={p.lineId}/><EditField name="instagram" label="Instagram" defaultValue={p.instagram}/></div>{message&&<div className={`notice ${message.includes("saved")?"notice--success":"notice--error"}`}>{message}</div>}<div className="participant-edit__actions"><button type="button" className="button button--ghost" disabled={busy} onClick={()=>{setEditing(false);setMessage("");}}>Cancel</button><button className="button button--primary" disabled={busy}>{busy?<span className="spinner"/>:<><Save size={14}/> Save information</>}</button></div></form>:<><div className="detail-grid"><Detail label="Student ID" value={p.studentId}/><Detail label="Role" value={p.participantKind === "performer" ? "Performer" : "Athlete / participant"}/><Detail label="Category / Type" value={p.category ?? "—"}/><Detail label={p.participantKind === "performer" ? "Performance" : "Sport"} value={p.sport}/><Detail label="Email" value={p.email ?? "—"}/><Detail label="Phone" value={p.phone ?? "—"}/><Detail label="Faculty" value={p.faculty}/><Detail label="Preferred contact" value={p.preferredContact ?? "—"}/><Detail label="LINE ID" value={p.lineId ?? "—"}/><Detail label="Instagram" value={p.instagram ? `@${p.instagram}` : "—"}/><Detail label="Nickname" value={`${p.nicknameThai ?? "—"} / ${p.nicknameEnglish ?? "—"}`}/><Detail label="Sex" value={p.sex ?? "—"}/></div>{message&&<div className={`notice ${message.includes("saved")||message.includes("updated")?"notice--success":"notice--error"}`}>{message}</div>}</>}
    ...
    <div className="drawer__actions"><button className="button button--soft" onClick={()=>void generateParticipantPdf([{participant:p,photoUrl:detail.photoUrl,nationalIdImageUrl:detail.nationalIdImageUrl,studentIdImageUrl:detail.studentIdImageUrl}], `${p.studentId}-${p.fullNameEnglish}`)}><FileDown size={14}/> Export PDF</button>{canEdit && <><button className="button button--soft" disabled={busy||editing} onClick={startEditing}><Pencil size={14}/> Edit information</button><button className="button button--soft" disabled={busy||editing} onClick={()=>void saveStatus("verified")}><Check size={14}/> Verify</button><button className="button button--danger" disabled={busy||editing} onClick={()=>void saveStatus("rejected")}>Request correction</button></>}</div>
...
function ImportPanel() {
  ...
  return <section className="content-card"><h2>Bring in your participant list</h2><p>Upload the CSV file. Existing participants are matched and updated by Student ID.</p><div className="drop-csv"><input id="csv" type="file" accept=".csv,text/csv" disabled={busy} onChange={e=>void handleFile(e.target.files?.[0])}/><label htmlFor="csv">{busy?<span className="spinner" style={{color:"#4b2f25"}}/>:<FileSpreadsheet size={33}/>}<strong>{busy?"Importing participants…":"Choose a CSV file"}</strong><span>UTF-8 CSV · Up to 100 rows processed per secure batch</span></label></div>{result&&<div className={`notice import-result ${result.startsWith("Imported")?"notice--success":"notice--error"}`}>{result}</div>}<div className="notice notice--info" style={{marginTop:16}}>Columns: <strong>Student ID, Name, Nickname, ชื่อ-สกุล, ชื่อเล่น, Sex, Faculty, Sport, Type, Phone number, Email</strong>. Use <strong>Parade</strong> (or <strong>ขบวนพาเหรด</strong>) in Sport or Type for normal parade members; they are classified as performers automatically. Phone numbers are cleaned to 10 digits beginning with 0; email whitespace is trimmed.</div></section>;
}
...
function mapParticipantRow(r:Record<string,string>,rowNumber:number){const fullNameEnglish=get(r,"Name").trim();const fullNameThai=get(r,"ชื่อ-สกุล").trim()||fullNameEnglish;const rawSport=get(r,"Sport").trim();const rawType=get(r,"Type").trim();const performerType=detectImportedPerformer(rawSport,rawType);return {studentId:get(r,"Student ID").trim(),fullNameEnglish,fullNameThai,nicknameEnglish:optional(get(r,"Nickname")),nicknameThai:optional(get(r,"ชื่อเล่น")),sex:optional(get(r,"Sex")),faculty:get(r,"Faculty").trim(),sport:performerType??rawSport,category:optional(rawType)||(performerType?"Performer":undefined),participantKind:performerType?"performer" as const:"athlete" as const,performerType,phone:cleanPhone(get(r,"Phone number","Phone Number"),rowNumber),email:optional(get(r,"Email").trim().toLowerCase())};}
