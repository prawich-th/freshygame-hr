...
function CreateParticipantDrawer({onClose, onCreated}: {onClose: () => void; onCreated: (id: Id<"participants">) => void}) {
  ...
  return <><div className="drawer-backdrop" onClick={busy ? undefined : onClose}/><aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="create-participant-title"><div className="drawer__head"><h2 id="create-participant-title">Create participant</h2><button disabled={busy} onClick={onClose} aria-label="Close"><X size={17}/></button></div><form className="participant-edit" onSubmit={submit}><p>Create one registration per sport or activity. Use the same Student ID to add another sport, then add document images for that registration.</p><ParticipantFields p={{}} editKind={editKind} setEditKind={setEditKind}/>{error && <div role="alert" className="notice notice--error">{error}</div>}<div className="participant-edit__actions"><button type="button" className="button button--ghost" disabled={busy} onClick={onClose}>Cancel</button><button className="button button--primary" disabled={busy}>{busy ? "Creating…" : "Create participant"}</button></div></form></aside></>;
}
