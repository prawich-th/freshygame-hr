...
import { RotatableDocumentPreview } from "./RotatableDocumentPreview";
import { ParticipantInformationFields } from "./ParticipantInformationFields";
import { participantCategories, categoryLabel } from "@/shared/participantCategories";
...
function StaffAuth() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    catch (e) { setError(errorMessage(e, "Sign in")); setBusy(false); }
  }
...
function StaffWorkspace({selectionPage, directoryPage}: {selectionPage: boolean; directoryPage: boolean}) {
  ...
  if (directoryPage) return <main className="directory-shell"><header className="directory-header"><Brand compact/><div>{current.role === "co-sport" ? <span>Co-sport</span> : <Link className="button button--soft" href="/staff" aria-label="Back to participants">Participants</Link>}<button className="button button--soft" onClick={() => void signOut()}>Sign out</button></div></header><ContactDirectory /></main>;
...
function ParticipantDashboard({ stats, selectedId, setSelectedId, canEdit }: { stats: { total:number;complete:number;pending:number;sports:number } | undefined; selectedId: Id<"participants"> | null; setSelectedId:(id:Id<"participants">|null)=>void; canEdit:boolean }) {
  ...
  async function exportSelectedSport(){if(!sport)return;setExporting(true);try{const entries=await convex.query(api.participants.exportSport,{sport});await generateParticipantPdf(entries,`Freshy-Game-${sport}`);}catch(error){setExportError(errorMessage(error,"Download sport PDF"));}finally{setExporting(false);}}
...
function ParticipantDrawer({ id, onClose, canEdit }: { id:Id<"participants">; onClose:()=>void; canEdit:boolean }) {
  ...
  const [files,setFiles] = useState<{profile?:File;nationalId?:File;studentId?:File}>({}); const [busy,setBusy]=useState(false); const [message,setMessage]=useState(""); const [messageIsError,setMessageIsError]=useState(false); const [editing,setEditing]=useState(false); const [editKind,setEditKind]=useState<ParticipantKind>("athlete");
  ...
  async function saveParticipant(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setMessage("");const data=new FormData(event.currentTarget);try{const performerType=editKind==="performer"?String(data.get("performerType")) as "Katakorn"|"Cheerleader"|"Parade":undefined;const faculty=String(data.get("faculty")) as "คณะแพทยศาสตร์"|"คณะศิลปศาสตร์"|"วิทยาลัยแพทยศาสตร์นานาชาติจุฬาภรณ์";await updateParticipant({participantId:id,participantKind:editKind,performerType,studentId:String(data.get("studentId")),fullNameThai:String(data.get("fullNameThai")),fullNameEnglish:String(data.get("fullNameEnglish")),nicknameThai:optional(String(data.get("nicknameThai"))),nicknameEnglish:optional(String(data.get("nicknameEnglish"))),sex:optional(String(data.get("sex"))),faculty,sport:editKind==="support"?"Support team":editKind==="performer"?(performerType??""):String(data.get("sport")),category:optional(String(data.get("category") ?? "")),categories:editKind==="athlete"?data.getAll("categories").map(String):undefined,email:optional(String(data.get("email"))),phone:optional(String(data.get("phone"))),lineId:optional(String(data.get("lineId"))),instagram:optional(String(data.get("instagram"))),nationalIdNumber:String(data.get("nationalIdNumber") ?? ""),birthDate:String(data.get("birthDate") ?? ""),guardianPhone:String(data.get("guardianPhone") ?? ""),emergencyContactName:String(data.get("emergencyContactName") ?? ""),emergencyContactRelationship:String(data.get("emergencyContactRelationship") ?? ""),drugAllergies:String(data.get("drugAllergies") ?? ""),foodAllergies:String(data.get("foodAllergies") ?? ""),hospitalizationHistory:String(data.get("hospitalizationHistory") ?? ""),jerseyNumber:String(data.get("jerseyNumber") ?? ""),medicalConditions:String(data.get("medicalConditions") ?? ""),preferredContact:optional(String(data.get("preferredContact")))});setMessageIsError(false);setMessage(p.signature ? "Participant information saved. Please ask the participant to sign again to confirm the updated information." : "Participant information saved");setEditing(false);}catch(e){setMessageIsError(true);setMessage(errorMessage(e, "Save changes"));}finally{setBusy(false);}}
  async function saveImageEdit(file: File, kind: UploadImageKind) {
    setBusy(true); setMessage("");
    try {
      const rotated = await compressImage(file, kind);
  ...
  async function staffUpload(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setMessage("");try{const upload=async(file:File|undefined,kind:UploadImageKind)=>{if(!file)return undefined;const compressed=await compressImage(file,kind);const url=await generateUploadUrl();const response=await fetch(url,{method:"POST",headers:{"Content-Type":compressed.type},body:compressed});if(!response.ok)throw new UserFacingError("Image upload failed");return (await response.json()).storageId as Id<"_storage">;};const profilePhotoId=await upload(files.profile,"profile");const nationalIdImageId=await upload(files.nationalId,"nationalId");const studentIdImageId=await upload(files.studentId,"studentId");await completeUpload({participantId:id,profilePhotoId,nationalIdImageId,studentIdImageId});setMessageIsError(false);setMessage("Identity documents saved");setFiles({});}catch(e){setMessageIsError(true);setMessage(errorMessage(e, "Upload images"));}finally{setBusy(false);}}
  return <><div className="drawer-backdrop" onClick={onClose}/><aside className="drawer"><div className="drawer__head"><h2>Participant record</h2><button onClick={onClose}><X size={17}/></button></div>
    ...
    {editing?<form className="participant-edit" onSubmit={saveParticipant}><ParticipantFields p={p} editKind={editKind} setEditKind={setEditKind}/>{message&&<div className={`notice ${!messageIsError?"notice--success":"notice--error"}`}>{message}</div>}<div className="participant-edit__actions"><button type="button" className="button button--ghost" disabled={busy} onClick={()=>{setEditing(false);setMessage("");}}>Cancel</button><button className="button button--primary" disabled={busy}>{busy?<span className="spinner"/>:<><Save size={14}/> Save information</>}</button></div></form>:<><div className="detail-grid"><Detail label="Student ID" value={p.studentId}/><Detail label="Role" value={kindLabel[participantKind(p)]}/><Detail label="Category / Type" value={categoryLabel(p) || "—"}/><Detail label={participantKind(p) === "support" ? "Team" : p.participantKind === "performer" ? "Performance" : "Sport"} value={p.sport}/><Detail label="Email" value={p.email ?? "—"}/><Detail label="Phone" value={p.phone ?? "—"}/><Detail label="Faculty" value={p.faculty}/><Detail label="Preferred contact" value={p.preferredContact ?? "—"}/><Detail label="LINE ID" value={p.lineId ?? "—"}/><Detail label="Instagram" value={p.instagram ? `@${p.instagram}` : "—"}/><Detail label="Nickname" value={`${p.nicknameThai ?? "—"} / ${p.nicknameEnglish ?? "—"}`}/><Detail label="Sex" value={p.sex ?? "—"}/><Detail label="Event-day jersey number / เลขเสื้อ" value={p.jerseyNumber || "Not provided"}/><Detail label="National ID number" value={p.nationalIdNumber || "Not provided"}/><Detail label="Age / อายุ" value={String(p.age ?? "Not provided")}/><Detail label="Date of birth" value={p.birthDate || "Not provided"}/><Detail label="Emergency contact name" value={p.emergencyContactName || "Not provided"}/><Detail label="Emergency contact relationship" value={p.emergencyContactRelationship || "Not provided"}/><Detail label="Emergency contact phone" value={p.guardianPhone || "Not provided"}/><Detail label="Drug allergies" value={p.drugAllergies || "Not provided"}/><Detail label="Food allergies" value={p.foodAllergies || "Not provided"}/><Detail label="Hospitalization / surgery history" value={p.hospitalizationHistory || "Not provided"}/><Detail label="Allergies / ประวัติการแพ้" value={p.allergies || "Not provided"}/><Detail label="Medical conditions / โรคประจำตัว" value={p.medicalConditions || "Not provided"}/></div>{message&&<div className={`notice ${!messageIsError?"notice--success":"notice--error"}`}>{message}</div>}</>}
    ...
    {canEdit && <form className="staff-upload" onSubmit={staffUpload}><h3>Update document images</h3><p>Uploads are linked to all registrations with this Student ID. Images are compressed automatically. The PDF adds a certified-copy statement to the ID images. Replacing either ID image clears the saved signature; the participant must sign the new copies.</p><div className="form-grid"><FileField label="Profile photo" onFile={file=>setFiles(current=>({...current,profile:file}))}/><FileField label="National ID card" onFile={file=>setFiles(current=>({...current,nationalId:file}))}/><div className="field field--wide"><FileField label="Student ID card" onFile={file=>setFiles(current=>({...current,studentId:file}))}/></div></div><button className="button button--primary" disabled={busy}>{busy?<span className="spinner"/>:"Save document images"}</button></form>}
  </aside></>;
}

function ParticipantFields({p, editKind, setEditKind, incomplete = false}: {incomplete?: boolean;p: Partial<Doc<"participants">>; editKind: ParticipantKind; setEditKind: (kind: ParticipantKind) => void}) {
  const [performerTeam, setPerformerTeam] = useState(p.performerType ?? "");
  return <div className="form-grid"><EditField name="studentId" label="Student ID" defaultValue={p.studentId} required inputMode="numeric" pattern="[0-9]{10}" minLength={10} maxLength={10} placeholder="6909680123"/><div className="field"><label>Role <span>*</span></label><select className="select" value={editKind} onChange={e=>setEditKind(e.target.value as ParticipantKind)}><option value="athlete">Athlete / participant</option><option value="performer">Performer</option><option value="support">Support team</option></select></div>{editKind==="performer"?<div className="field field--wide"><label>Performer team <span>*</span></label><select className="select" name="performerType" required value={performerTeam} onChange={e => setPerformerTeam(e.target.value as typeof performerTeam)}><option value="" disabled>Select team</option><option value="Katakorn">Katakorn</option><option value="Cheerleader">Cheerleader</option><option value="Parade">Parade</option></select></div>:editKind === "athlete" ? <SportFields sport={p.sport} categories={participantCategories(p)}/> : null}<EditField name="fullNameThai" label="Thai full name" defaultValue={p.fullNameThai} required={!incomplete}/><EditField name="nicknameThai" label="Thai nickname" defaultValue={p.nicknameThai}/><EditField name="fullNameEnglish" label="English full name" defaultValue={p.fullNameEnglish} required={!incomplete}/><EditField name="nicknameEnglish" label="English nickname" defaultValue={p.nicknameEnglish}/><div className="field"><label>Faculty {!incomplete && <span>*</span>}</label><select className="select" name="faculty" required={!incomplete} defaultValue={p.faculty ?? ""}><option value="" disabled>Select faculty</option><option value="คณะแพทยศาสตร์">คณะแพทยศาสตร์</option><option value="คณะศิลปศาสตร์">คณะศิลปศาสตร์</option><option value="วิทยาลัยแพทยศาสตร์นานาชาติจุฬาภรณ์">วิทยาลัยแพทยศาสตร์นานาชาติจุฬาภรณ์</option></select></div><EditField name="sex" label="Sex" defaultValue={p.sex}/>{editKind === "support" && <div className="field"><label htmlFor="support-subtype">Support subtype <span>*</span></label><select id="support-subtype" className="select" name="category" required defaultValue={participantKind(p) === "support" ? p.category ?? "" : ""}><option value="" disabled>Select subtype</option>{SUPPORT_TYPES.map(type => <option key={type}>{type}</option>)}</select></div>}{editKind === "performer" && (performerTeam === "Parade" ? <div className="field"><label>Parade member type *</label><select className="select" name="category" required defaultValue={p.category ?? ""}><option value="" disabled>Select type</option>{p.category && !PARADE_TYPES.some(type => type === p.category) && <option>{p.category}</option>}{PARADE_TYPES.map(type => <option key={type}>{type}</option>)}</select></div> : <EditField name="category" label="Category / type" defaultValue={participantKind(p) === "performer" ? p.category : ""}/>)}<EditField name="phone" label="Phone" defaultValue={p.phone} type="tel" required={incomplete}/><EditField name="email" label="Email" defaultValue={p.email} type="email"/><EditField name="preferredContact" label="Preferred contact" defaultValue={p.preferredContact}/><EditField name="lineId" label="LINE ID" defaultValue={p.lineId}/><EditField name="instagram" label="Instagram" defaultValue={p.instagram}/><ParticipantInformationFields value={p} includeJersey={editKind !== "performer"}/></div>;
}
...
function CreateParticipantDrawer({onClose, onCreated}: {onClose: () => void; onCreated: (id: Id<"participants">) => void}) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const id = await create({incomplete, participant: {
        ...
        sport: editKind === "support" ? "Support team" : performerType ?? value("sport"), category: optional(value("category")), categories: editKind === "athlete" ? data.getAll("categories").map(String) : undefined,
        nicknameThai: optional(value("nicknameThai")), nicknameEnglish: optional(value("nicknameEnglish")), sex: optional(value("sex")),
        nationalIdNumber: optional(value("nationalIdNumber")), birthDate: optional(value("birthDate")), guardianPhone: optional(value("guardianPhone")), emergencyContactName: optional(value("emergencyContactName")), emergencyContactRelationship: optional(value("emergencyContactRelationship")), drugAllergies: optional(value("drugAllergies")), foodAllergies: optional(value("foodAllergies")), hospitalizationHistory: optional(value("hospitalizationHistory")), jerseyNumber: optional(value("jerseyNumber")), medicalConditions: optional(value("medicalConditions")),
      ...
      onCreated(id);
    } catch (e) { setError(errorMessage(e, "Create the participant")); }
    finally { setBusy(false); }
  }
  return <><div className="drawer-backdrop" onClick={busy ? undefined : onClose}/><aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="create-participant-title"><div className="drawer__head"><h2 id="create-participant-title">Create participant</h2><button disabled={busy} onClick={onClose} aria-label="Close"><X size={17}/></button></div><form className="participant-edit" onSubmit={submit}><p>Create one registration per sport or activity. Use the same Student ID to add another sport, existing documents are linked automatically.</p>{me?.role === "admin" && <label className="notice notice--info participant-edit__incomplete"><input type="checkbox" checked={incomplete} onChange={e => setIncomplete(e.target.checked)}/> Create an incomplete profile</label>}{incomplete && <p>Student ID, registered phone, and sport/activity are required. The athlete will complete their names and faculty and confirm their details when uploading documents.</p>}<ParticipantFields p={{}} editKind={editKind} setEditKind={setEditKind} incomplete={incomplete}/>{error && <div role="alert" className="notice notice--error">{error}</div>}<div className="participant-edit__actions"><button type="button" className="button button--ghost" disabled={busy} onClick={onClose}>Cancel</button><button className="button button--primary" disabled={busy}>{busy ? "Creating…" : "Create participant"}</button></div></form></aside></>;
}
...
function ImportPanel() {
  ...
  async function handleFile(file?: File) {
    ...
    let created = 0, updated = 0, batchStart = 0;
    try {
      const rows = parseCsv(await file.text());
      if (!rows.length) throw new UserFacingError("The CSV has no participant rows. Add participants below the template headers and upload it again.");
      const mapped = rows.map((row, index) => {
        const p = mapParticipantRow(row, index + 2);
        if (!/^\d{10}$/.test(p.studentId)) throw new UserFacingError(`Row ${index + 2}: Student ID must contain exactly 10 digits. Check the Student ID column.`);
        if (!p.fullNameEnglish) throw new UserFacingError(`Row ${index + 2}: English name is missing. Fill in the Name column.`);
        if (!["คณะแพทยศาสตร์", "คณะศิลปศาสตร์", "วิทยาลัยแพทยศาสตร์นานาชาติจุฬาภรณ์"].includes(p.faculty)) throw new UserFacingError(`Row ${index + 2}: Faculty is not supported. Use the Thai faculty name shown in the participant form.`);
        if (!p.sport) throw new UserFacingError(`Row ${index + 2}: Sport Code is missing. Use a code from the table below.`);
        return p;
      });
      for (let i = 0; i < mapped.length; i += 100) {
        batchStart = i + 2;
        const batch = await importBatch({participants: mapped.slice(i, i + 100)});
        created += batch.created; updated += batch.updated;
      }
      setResult(`Imported ${mapped.length} rows — ${created} created, ${updated} updated.`);
    } catch (error) {
      setResult(`${errorMessage(error, "Import participants")} ${created + updated > 0 ? `${created} participants created and ${updated} updated before this problem.` : "No earlier batches were saved."}${batchStart ? ` Import stopped at the batch starting at row ${batchStart}. Check the participant list before retrying; matching Student IDs will be updated.` : " Correct the CSV and upload it again."}`);
    } finally { setBusy(false); }
  }
...
function StaffPanel() {
  ...
  async function changeAccess(args: Parameters<typeof update>[0]) {
    setSavingAccess(true); setAccessError("");
    try { await update(args); }
    catch (error) { setAccessError(errorMessage(error, "Change staff access")); }
    finally { setSavingAccess(false); }
  }
  return <section className="content-card"><h2>Staff accounts</h2>{accessError && <div role="alert" className="notice notice--error">{accessError}</div>}<p>Only administrators can change access. Co-sport can search contact and team information only. Registrars can edit sensitive records; viewers cannot retrieve National ID or Student ID card images.</p><table className="staff-list"><thead><tr><th>Staff member</th><th>Role</th><th>Account</th></tr></thead><tbody>{staff?.map(s=><tr key={s.id}><td><strong>{s.name}</strong><br/><span style={{color:"#8d837e"}}>{s.email}</span></td><td><select className="select" disabled={savingAccess} value={s.role} onChange={e=>void changeAccess({userId:s.id,role:e.target.value as typeof s.role,active:s.active})}><option value="admin">Admin</option><option value="registrar">Registrar</option><option value="viewer">Viewer</option><option value="co-sport">Co-sport</option></select></td><td><button disabled={savingAccess} className={`button ${s.active?"button--soft":"button--danger"}`} onClick={()=>void changeAccess({userId:s.id,role:s.role,active:!s.active})}>{s.active?"Active":"Inactive"}</button></td></tr>)}</tbody></table></section>;
}
...
function parseCsv(text:string){const rows:string[][]=[];let row:string[]=[];let cell="";let quoted=false;for(let i=0;i<text.length;i++){const c=text[i];if(c==='"'){if(quoted&&text[i+1]==='"'){cell+='"';i++;}else quoted=!quoted;}else if(c===","&&!quoted){row.push(cell.trim());cell="";}else if((c==="\n"||c==="\r")&&!quoted){if(c==="\r"&&text[i+1]==="\n")i++;row.push(cell.trim());if(row.some(Boolean))rows.push(row);row=[];cell="";}else cell+=c;}if(cell||row.length){row.push(cell.trim());rows.push(row);}if(quoted)throw new UserFacingError("The CSV has an unclosed quote. Export it as CSV again from your spreadsheet and retry.");const headers=rows.shift()?.map(h=>h.replace(/^\uFEFF/,""))??[];return rows.map(values=>Object.fromEntries(headers.map((h,i)=>[h,values[i]??""])));}
...
function cleanPhone(value:string,rowNumber:number){if(!value.trim())return undefined;let phone=value.replace(/\D/g,"");if(phone.startsWith("66")){const localNumber=phone.slice(2);phone=localNumber.startsWith("0")?localNumber:`0${localNumber}`;}else if(phone.length===9&&!phone.startsWith("0")){phone=`0${phone}`;}if(!/^0\d{9}$/.test(phone))throw new UserFacingError(`Row ${rowNumber}: Phone number could not be cleaned to a valid 10-digit Thai number`);return phone;}
function mapParticipantRow(r:Record<string,string>,rowNumber:number){const fullNameEnglish=get(r,"Name").trim();const fullNameThai=get(r,"ชื่อ-สกุล").trim()||fullNameEnglish;const rawSport=(r["Sport Code"] ?? r["sportCode"] ?? r["sport_code"] ?? get(r,"Sport")).trim();const rawType=get(r,"Type").trim();const isSupport=participantKind({sport:rawSport,category:rawType}) === "support";const performerType=isSupport?undefined:detectImportedPerformer(rawSport,rawType);return {studentId:get(r,"Student ID").trim(),fullNameEnglish,fullNameThai,nicknameEnglish:optional(get(r,"Nickname")),nicknameThai:optional(get(r,"ชื่อเล่น")),sex:optional(get(r,"Sex")),faculty:get(r,"Faculty").trim(),sport:isSupport?"Support team":performerType??normalizeSport(rawSport),category:optional(rawType)||(performerType?"Performer":undefined),categories:!isSupport&&!performerType?rawType.split(";").map(value=>value.trim()).filter(Boolean):undefined,participantKind:isSupport?"support" as const:performerType?"performer" as const:"athlete" as const,performerType,phone:cleanPhone(get(r,"Phone number","Phone Number"),rowNumber),email:optional(get(r,"Email").trim().toLowerCase()),instagram:optional(get(r,"Instagram","IG").trim().replace(/^@/,"")),lineId:optional(get(r,"LINE ID","lineId","line_id")),preferredContact:optional(get(r,"Preferred contact method","Preferred method","Preferred contact","preferredContact","preferred_contact"))};}
