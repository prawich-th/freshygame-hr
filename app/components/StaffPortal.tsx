...
function ParticipantDrawer({ id, onClose, canEdit }: { id:Id<"participants">; onClose:()=>void; canEdit:boolean }) {
  ...
  if (!detail) return <><div className="drawer-backdrop" onClick={onClose}/><aside className="drawer drawer--participant"><span className="spinner" style={{color:"#4b2f25"}}/></aside></>;
  ...
  return <><div className="drawer-backdrop" onClick={onClose}/><aside className="drawer drawer--participant"><div className="drawer__head"><h2>Participant record</h2><button onClick={onClose} aria-label="Close participant record"><X size={17}/></button></div>
    ...
    <div className="record-actions">
      <div role="group" aria-label="Record tools">
        <button className="button button--soft" disabled={busy || pdfState === "loading"} aria-busy={pdfState === "loading"} onClick={() => void downloadPdf()}>
      ...
      {canEdit && <div role="group" aria-label="Review decisions">
        <button className="button button--soft" disabled={busy || editing} onClick={() => setCorrectionsOpen(true)} aria-haspopup="dialog" aria-expanded={correctionsOpen}>
          <Pencil size={14} />Request corrections{p.correctionRequests?.length ? ` (${p.correctionRequests.length})` : ""}
        </button>
        <button className="button button--primary" disabled={busy || editing} onClick={() => void saveStatus("verified")}><Check size={14} />Verify record</button>
      </div>}
    </div>
    {editing?<form className="participant-edit" onSubmit={saveParticipant}><ParticipantFields p={p} editKind={editKind} setEditKind={setEditKind}/>{message&&<div className={`notice ${!messageIsError?"notice--success":"notice--error"}`}>{message}</div>}<div className="participant-edit__actions"><button type="button" className="button button--ghost" disabled={busy} onClick={()=>{setEditing(false);setMessage("");}}>Cancel</button><button className="button button--primary" disabled={busy}>{busy?<span className="spinner"/>:<><Save size={14}/> Save information</>}</button></div></form>:<><ParticipantRecordDetails participant={p} role={kindLabel[participantKind(p)]} activityLabel={participantKind(p) === "support" ? "Team" : participantKind(p) === "performer" ? "Performance" : "Sport"} category={categoryLabel(p)} />{message&&<div className={`notice ${!messageIsError?"notice--success":"notice--error"}`}>{message}</div>}</>}
    {canEdit && correctionsOpen && <FieldCorrectionEditor participant={p} onClose={() => setCorrectionsOpen(false)} />}
    <section className="record-section record-documents"><header><h3>Documents</h3><p>Profile photo and identification copies</p></header>
