...
export function SportCatalog() {
  ...
  return <section className="content-card sport-catalog">
    ...
    {pending && <div className="notice notice--info" role="region" aria-label="Migration preview"><h3>Review migration</h3><p><strong>Old value:</strong> {pending.category?.oldName ?? pending.expectedName}<br/><strong>New value:</strong> {pending.category?.name ?? pending.name}</p><p>{pending.category ? preview?.categories.find(c => c.name === pending.category?.oldName)?.participants ?? 0 : preview?.participants ?? 0} linked registrations will use the new value. Old inputs will resolve to the new value. Existing signatures on changed registrations will be cleared for re-signing.</p><button className="button button--primary" disabled={busy || !preview} onClick={() => void apply(pending)}>{busy ? "Migrating…" : "Apply migration"}</button> <button className="button button--ghost" disabled={busy} onClick={() => setPending(null)}>Cancel</button></div>}
