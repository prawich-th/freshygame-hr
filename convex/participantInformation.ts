...
type Information = { jerseyNumber?: string; allergies?: string; medicalConditions?: string; nationalIdNumber?: string; birthDate?: string; guardianPhone?: string; drugAllergies?: string; foodAllergies?: string; hospitalizationHistory?: string; sex?: string };

export function normalizeInformation(input: Information, required = false) {
  const result: Information & { age?: number } = {};
  for (const key of ["jerseyNumber", "nationalIdNumber", "birthDate", "guardianPhone", "drugAllergies", "foodAllergies", "medicalConditions", "hospitalizationHistory", "sex"] as const) {
    if (input[key] === undefined && !required) continue;
    let value = input[key]?.trim() ?? "";
    if (required && !value) throw new ConvexError(`Please complete ${key}. Your jersey number and personal information are required. Enter None for health histories that do not apply.`);
