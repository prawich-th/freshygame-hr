...
test("contact export enforces staff access and paginates across other participant types", async () => {
  ...
  const ids = await t.run(async ctx => {
    ...
    await ctx.db.insert("participants", { ...base, studentId: "001", nicknameThai: "ชื่อเล่น", phone: "0812345678", lineId: "test.line" });
    await ctx.db.insert("participants", { ...base, studentId: "002", participantKind: "performer", sport: "Parade" });
  ...
  expect(third.page).toEqual([{ studentId: "001", name: "ทดสอบ", nickname: "ชื่อเล่น", faculty: "Medicine", tel: "0812345678", line: "test.line" }]);
  ...
  expect(performers.page.map(p => p.studentId)).toEqual(["002"]);
});

test("CSV handles Thai, quoting, blank fields, formulas and repeated registrations", () => {
  const person = { studentId: "0012345678", name: 'ทดสอบ, "Name"', nickname: "A\nB", faculty: "", tel: "0812345678", line: "=1+1" };
  const csv = participantCsv([person, person]);
  expect(csv).toBe('\uFEFF"Name","Nickname","Faculty","Tel","Line","Student ID"\r\n"ทดสอบ, ""Name""","A\nB","","0812345678","\'=1+1","0012345678"\r\n');
  expect(participantCsv([])).toContain('"Student ID"\r\n');
});
